...
/**
 * `apps/dashboard/src/shared/callsign.ts`-only Vitest config, used
 * exclusively by Stryker (stryker.dashboard-callsign.config.mjs) — NOT
 * wired into `pnpm run test`, which keeps using the root config's
 * full-workspace run.
 *
 * Mirrors vitest.dashboard-fleet.config.ts's reasoning: callsign.ts has
 ...
 * fleet.test.ts's `describe('firingCallsign', ...)` block (fleet.ts
 * re-exports `firingCallsign`/`CALLSIGN_WORDS` from this module). Scoping

 ...
 *
 * Mirrors vitest.ci-detect-flaky.config.ts's reasoning: validateQuarantineList
 * and summarizeQuarantine have zero imports beyond Node built-ins and are
 * exercised with concrete expected-output assertions by
 * quarantine-report.test.ts. Scoping to just that one test file sidesteps the

 ...
 * stryker.ci-check-conflict-markers.config.mjs and stryker.ci-detect-flaky.config.mjs.
 * Only `validateQuarantineList` and `summarizeQuarantine` are mutated; `main`
 * carries a `// Stryker disable all` comment in the source since it reads the
 * real quarantine file from disk and calls `process.exit`, the same stance
 * the other four ci/ configs take for their own impure glue.
 *
 * A surviving mutant here would let an owner-less or reason-less quarantine
 * entry slip past the CI gate silently, or misreport the quarantined-test
 * tally — exactly the drift the config/quarantine/flaky-tests.json gate
 * exists to catch (PATTERNS-AND-STANDARDS §4, "validators-as-gates").
 *
 * Same shape of good target as stryker.ci-detect-flaky.config.mjs:

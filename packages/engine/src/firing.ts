...
export interface FiringDeps {
  ...
  /** Commit-time independent review (see the section at the end of this
   *  file) — absent means the firing goes unreviewed. */
  readonly reviewer?: CommitReviewPort;
}
...
export async function runFiring(
  ...
  // ---- commit-time independent review (docs/BACKLOG-999.md C5) ----
  // Gate-passed work only: reverted work is gone, and unverified work has a
  // bigger problem than a reviewer can name. Non-blocking — the verdict above
  // is already final; the findings only ride along on the record.
  const review: CommitReview | null =
    gateResult === 'passed' && deps.reviewer
      ? await runCommitReview(deps.reviewer, {
          headBefore,
          headAfter,
...
// ---- commit-time independent review (docs/BACKLOG-999.md §L C5) -----------
// The gate is deterministic — typecheck, tests, build, the diff-size check —
// and catches nothing a fresh reader of the diff would. This is the SOTA-MAP
// C5 second stage: ONE cheap, tool-less, fresh-context model call per
// gate-passed firing whose only instruction is to find problems in the diff.
// NON-BLOCKING by design (the map's own rule): a finding never changes the
// gate verdict and never reverts a commit; it rides on the firing record and
// the flight log. Anything that must not ship belongs in the gate, not here.

...
describe('buildFiringPrompt', () => {
  ...
  it('exposes a stable version tag for telemetry', () => {
    expect(FIRING_PROMPT_VERSION).toBe('firing-v14');
  });
  ...
  it('pins the exact output for a fully-populated firing (normal board, retro, all optional sections)', () => {
    ...
    expect(p.split('\n')).toEqual([
      ...
      '   `Model: <your exact model id>`, `Firing-Prompt-Version: firing-v14`,',
      ...
      '- A red-main / convergence-red signal may be STALE — it judged a SHA a later commit already',
      '  fixed. Never react with your own git revert walk; propose a board task with the evidence',
      '  and default to WAITING instead (docs/debriefs/2026-09-06-red-main-revert-cascade.md).',
      '- Uncommitted changes already in the tree at start (no `wip(autopilot): checkpoint` commit)',
      "  may be a live sibling's work sharing this checkout, not only a turn-capped prior firing of",
      '  yours — re-check `git status`/`git log` right before your final commit; if it already',
      '  landed under a different commit, that is a clean noop, never a cue to retry',

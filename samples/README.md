...
| [`static-site/`](static-site/) | Static HTML/CSS, no build tooling | `static-site` (`packages/onboarding/src/gate/detectors/static-site.ts`) — `.html`/`.htm` files with no `package.json` | lint (`html-validate`) + test (`linkinator`, link-checking), both via `npx --yes`; no build/typecheck (a static site needs neither) | **Verified** — `npx --yes html-validate "**/*.html" && npx --yes linkinator . --recurse` both pass standalone |

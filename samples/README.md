...
| [`calculator/`](calculator/) | Static HTML + vanilla JS, npm-tested | `js` — `package.json` `scripts.test` | `node --test` acceptance suite | **Seeded — awaiting autonomous flight**: the 12 red tests in `calc.test.js` ARE the mission endpoint ([`MISSION.md`](calculator/MISSION.md)); open `index.html` before/after to judge the flight in ten seconds |

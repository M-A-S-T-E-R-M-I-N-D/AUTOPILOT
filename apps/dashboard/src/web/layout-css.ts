...
/* OTA UPDATE BANNER (operator ask 2026-09-05): a sticky strip ABOVE the
   masthead, success-voiced (an update is good news, not an alarm), visible
   from every scroll position — that is the "sticky" in the ask. Hidden by
...
/* …and that cluster is ONE segmented control, not five floating dots: the
   ...
   only the outer corners rounded. Narrow is the default (mobile-first — the
   md rule below spaces them back into separate pills once there is room). */
.masthead-icons { display: flex; flex-wrap: nowrap; align-items: center; }
.masthead-icons > details > summary { border-radius: 0; margin-inline-start: -1px; }
.masthead-icons > details:first-child > summary { border-start-start-radius: var(--radius-full); border-end-start-radius: var(--radius-full); margin-inline-start: 0; }
.masthead-icons > details:last-child > summary { border-start-end-radius: var(--radius-full); border-end-end-radius: var(--radius-full); }
/* An open or hovered cell lifts above its neighbours so its full border is
   drawn, not clipped by the next cell's overlap. */
.masthead-icons > details > summary:hover,
.masthead-icons > details > summary:focus-visible,
.masthead-icons > details[open] > summary { position: relative; z-index: 1; }
@media (min-width: 48rem) {
  .masthead-icons { gap: var(--space-2); }
  .masthead-icons > details > summary { border-radius: var(--radius-full); margin-inline-start: 0; }
}
...
/* HUE (epic 0029 slice 7, operator 2026-09-13: "a full hue for the whole
   design, not only the phosphor"): the rotation is COMPUTED BY THE CLIENT
   (web/features/prefs.ts) from the --color-*-base twins the tokens emit, and
   written as inline --color-* on <html> — never through relative colour
   syntax: this Chromium answers CSS.supports('color', 'oklch(from …)') with
   false, and an unparseable custom property at computed time made every
   colour fall back to the UA default, so the terminal theme "vanished" the
   moment a hue was set. Surfaces, text, borders, the accent, info, the low
   severity and needs-you rotate; success, warning, danger and the other
   severities keep their meaning. The rows below style the control. */
...
.landing-actions { display: flex; flex-wrap: wrap; gap: var(--space-2); justify-content: flex-end; margin-top: var(--space-3); }
...
.release-actions { display: flex; flex-wrap: wrap; gap: var(--space-2); justify-content: flex-end; margin-top: var(--space-3); }
...
.pr-review-actions { gap: var(--space-2); }
...
.issue-triage-actions { display: flex; flex-wrap: wrap; gap: var(--space-2); justify-content: flex-end; margin-top: var(--space-2); }
...
.mirror-pass-actions { display: flex; flex-wrap: wrap; gap: var(--space-2); justify-content: flex-end; margin-top: var(--space-2); }
...
.discussions-triage-panel { background: var(--color-surface-raised); border: 1px solid var(--color-border); border-radius: var(--shape-medium); padding: var(--space-3) var(--space-4); box-shadow: var(--elevation-level-1); }
.discussions-triage-title { margin: 0 0 var(--space-2); font-size: var(--text-base); }
.discussions-triage-list { display: flex; flex-direction: column; }
.discussions-triage-item { padding: var(--space-2) 0; border-top: 1px solid var(--color-border); font-size: var(--text-sm); }
.discussions-triage-item:first-of-type { border-top: none; }
.discussions-triage-actions { display: flex; flex-wrap: wrap; gap: var(--space-2); justify-content: flex-end; margin-top: var(--space-2); }
.discussions-triage-execute { font: inherit; font-size: var(--text-sm); cursor: pointer; padding: var(--space-1) var(--space-3); border-radius: var(--shape-extra-small); border: 1px solid var(--color-accent); background: var(--color-accent); color: var(--color-accent-text); }
.discussions-triage-execute:disabled { opacity: 0.6; cursor: default; }
.discussions-triage-result { margin-top: var(--space-1); font-size: var(--text-sm); text-align: end; }
.discussions-triage-result:empty { display: none; }
.discussions-triage-result-ok { color: var(--color-success); }
.discussions-triage-result-fail { color: var(--color-sev-critical); }
...
.card-actions { display: flex; flex-wrap: wrap; column-gap: var(--space-2); justify-content: flex-end; margin-top: var(--space-1); }
...
.ritual-actions { display: flex; flex-wrap: wrap; gap: var(--space-2); justify-content: flex-end; }
...
/* KEEPER QUEUE (epic 0021 slice 4): one list of everything waiting on a
   human — a view the nav module builds over the panels' own items, so it
...
.keeper-queue-item:last-child { border-block-end: 0; }
.keeper-queue-source { grid-area: source; min-inline-size: 4.5rem; font-size: var(--text-xs); font-weight: 700; letter-spacing: 0.06em; text-transform: uppercase; color: var(--color-text-muted); }
.keeper-queue-open { grid-area: open; background: none; border: 0; padding: 0; min-block-size: 1.5rem; font: inherit; color: var(--color-text); text-align: start; cursor: pointer; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }

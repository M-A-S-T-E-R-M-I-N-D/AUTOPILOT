...
describe("reconstructing shell.ts's one remaining bundle-composing function byte-for-byte from segments + slots", () => {
  ...
  /**
   * discussionsTriageJs's own reconstruction, from its real file under
   * web/features/. It carries five real relative-import splices of its own
   * (discussionsTriageDecisionLabel/discussionsTriageItems/
   * discussionsTriageCanExecute/discussionsTriageConfirmMessage/
   * discussionsTriageExecuteResultMessage from ../discussions-triage-panel.js),
   * resolved against web/features/ rather than SHELL_DIR, and no non-splice
   * slots at all.
   */
  async function reconstructDiscussionsTriageJs(): Promise<string> {
    const discussionsTriageSource = readFileSync(DISCUSSIONS_TRIAGE_TS, 'utf8');
    const spliceEntries = findSpliceManifest(discussionsTriageSource, DISCUSSIONS_TRIAGE_TS);
    const resolvedBindings = await resolveManifestBindings(spliceEntries, FEATURES_DIR);
    return (
      await assembleFunctionFromManifest(
        discussionsTriageSource,
  ...
  it('discussionsTriageJs: assembleFromManifest reproduces the real function output from a pre-built manifest built off discussions-triage.ts', async () => {
    const discussionsTriageSource = readFileSync(DISCUSSIONS_TRIAGE_TS, 'utf8');
    const manifest = buildAssemblyManifest(discussionsTriageSource, DISCUSSIONS_TRIAGE_TS, [
      'discussionsTriageJs',
    ]);
    const resolvedBindings = await resolveManifestBindings(manifest.entries, FEATURES_DIR);
    const reassembled = (

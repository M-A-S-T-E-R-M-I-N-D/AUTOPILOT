...
/**
 * Unit coverage for `handleReportComposeTasks`'s request guard (method,
 * Content-Type, body size, JSON parse, description/contextJson/moduleSources
 * validation, rate limit, missing api) — same shape
 * `test/server/report-compose.test.ts` exercises for the sibling
 * single-report `/api/report/compose` guard.
 */
...
} from '../../src/server/report-compose-tasks.js';
...
describe('handleReportComposeTasks', () => {
  const call = (
    req: ReturnType<typeof fakeRequest>,
    res: ReturnType<typeof fakeResponse>,
    api: ReportComposeTasksApi | undefined,
    limiter: RateLimiter,
  ): Promise<void> => handleReportComposeTasks(req as never, res as never, api, {}, limiter);
  ...
  it('returns 429 when the rate limit is exceeded, without calling the api', async () => {
    const api: ReportComposeTasksApi = vi.fn();
    const res = fakeResponse();
  ...
  it('passes description, contextJson, and moduleSources through to the api and returns its result', async () => {
    const api: ReportComposeTasksApi = vi.fn().mockResolvedValue({
      ok: true,
      tasks: [{ title: 't', body: 'b', severity: 'high', dimension: 'correctness' }],
    });
    const req = fakeRequest({ method: 'POST', contentType: 'application/json' });
    const res = fakeResponse();
    await postJson((r, s) => call(r as never, s as never, api, fakeLimiter(true)), req, res, {
    ...
    expect(readBody(res)).toMatchObject({ ok: true, tasks: [{ title: 't' }] });
  });

  it('defaults an absent moduleSources to an empty array', async () => {
    const api: ReportComposeTasksApi = vi.fn().mockResolvedValue({ ok: false, reasoning: 'x' });
    const req = fakeRequest({ method: 'POST', contentType: 'application/json' });
    const res = fakeResponse();
    await postJson((r, s) => call(r as never, s as never, api, fakeLimiter(true)), req, res, {
      description: 'note',
  ...
  it('returns 500 when the api throws', async () => {
    const api: ReportComposeTasksApi = vi.fn().mockRejectedValue(new Error('boom'));
    const req = fakeRequest({ method: 'POST', contentType: 'application/json' });
    const res = fakeResponse();
    await postJson((r, s) => call(r as never, s as never, api, fakeLimiter(true)), req, res, {
      description: 'note',

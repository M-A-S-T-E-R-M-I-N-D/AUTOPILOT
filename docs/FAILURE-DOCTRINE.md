...
| 28 | Operator-environment detail published in a PUBLIC commit log — subjects describing the maintainer's own hardware state and the time of day it happened | commitlint refuses wall-clock times and machine-state phrases in a subject (warning, never blocking a WIP checkpoint); session detail belongs in a debrief | `commitlint.config.js` `subject-no-operator-environment` |

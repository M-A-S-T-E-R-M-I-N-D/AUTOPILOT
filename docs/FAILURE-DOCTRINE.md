...
| 34 | A sync-back published whatever sat at the lane's HEAD — including a commit the gate had refused to judge (`unverifiable`: a second file left uncommitted beside it, nothing reverted). The flight branch's formatter check went red one lane later, and the landing would have refused the rung (four-lane rung, 2026-09-19) | only a verified head is published: the lane carries one bit across firings — has a gate judged the tree at HEAD green — moved by each record (green sets it, a revert keeps it, a checkpoint takes its own telemetry gate, an unverifiable commit clears it), persisted by sha, and consulted by every sync-back: per firing, at flight end (which then takes the stranded-work path: log, event, inbox task) and at the next launch's catch-up, where a parked head stays parked only while it is the same commit | `apps/dashboard/src/flight/lane-head.ts` + its census over fly.ts; mutation config `dashboard-lane-head` |

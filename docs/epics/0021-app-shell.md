...
| 3 | **Plan canvas** (the node workflow): the gate pipeline / flight plan as an editable graph — palette · canvas · properties; pan/zoom, drag, tap-tap connect on touch, deterministic auto-layout, undo/redo, autosave to config, read-only for visitors. Grows from `pipeline-panel.ts`'s SVG canvas + tree; no new dependencies | **second cut shipped** — `e72093c6`: the FLIGHT PLAN EDITOR under Plan — the gate as a chain of step nodes, the selected one editable (on/off, command, label), autosave to a local draft, Publish through `POST /api/plan/publish` (`plan-guard.ts` validates, `setProjectGateConfig` stores; the next landing and firing run it), Discard, read-only where the route is not served. First cut (the camera) was `b634e421`. Undo/redo shipped next (Ctrl+Z / Ctrl+Shift+Z, two buttons; a Discard is undoable). Remaining: drag-to-reorder once the engine honours an order, tap-tap connect |
| 4 | **Keeper as an inbox**: one queue of everything waiting on a human — PR verdicts, triage, approvals, proposals — each item carrying the accumulated context (what ran, what this step does, what comes next), settled items collapsing to a badge history (0018 slice 3's Keeper tab, done as a subject) | **shipped** — `5af2cc8a`: the queue — every waiting item (PR verdicts, pool, triage, mirror findings, backlog, wisdom, approvals) as one list with source · what · why · one proxied exit action · Open; j/k/arrows, Enter, a; a view over the panels (the badge counts the same items); guarded, never built empty. First cut (the live count) was `b634e421`. Settled history shipped in `aea12400`: every key the session listed and no longer waits is the settled count on the title; when the last one settles the queue stays to say so |
...
| 6 | **Context rail at expanded**: the supporting pane — live lanes + the Keeper queue beside whatever subject is open (M3 supporting-pane canonical layout) | **shipped** — `8a1650b0`: from xl the fleet page gains the supporting pane (lanes in flight + the Keeper queue, sticky, own scroll; sections move in at xl and back below it; a project page never grows one). Same commit: tablet Playwright projects (iPad Mini 768 portrait = md, 1024 landscape = lg) with their own spec and CI-canonical baselines |
...
| 9 | **Board as columns**: the task list read as Queued · In flight & needs you · Done — CSS grid with dense auto-flow and a status attribute per row, no DOM reorder (drag, ↑/↓ and screen readers keep the one priority order); a count on each column head; columns from lg by default, never below md; a remembered Columns/List toggle. Research: Linear's board as a layout over the list, WIP as a soft visible count, a needs-you column as the human gate (kanban-for-agents guides, 2026) | **shipped** — `05f34cec` |
| 10 | **RTL & density audit** (operator, 2026-09-12: "sometimes we build right-to-left and it looks strange"): measured in Hebrew at 412/768/1024/1280 on live data. The stylesheet was already all-logical; the defects were user content in the wrong direction (bidi laws: `unicode-bidi: plaintext` on titles and inputs, LTR for code/commands/paths), masthead popovers overflowing a phone (a viewport sheet below md), a 13k-px Plan subject (bounded span tree and canvas, editor first), pool rows squeezed in the rail (stacked), and a skip link parked off-screen physically (clipped). Proved by unit laws and a phone e2e in both directions | **shipped** — `28f07e54` |
| 11 | **Desktop width & every theme** (operator, 2026-09-12: "on desktop we have more width"; "many things are not adapted per theme"): measured at 1280/1440 against the fixture server. A project page's Overview reads as two columns from lg — the project card beside its operational stack, DOM order kept within each; five stat tiles fit the column the rail leaves at 1280; the board's "auto" view is a list until two of its three columns would have rows (columns show flow); the rail label no longer ellipsises; reorder arrows show on a row's hover/focus with a hover-capable pointer. Themes: an oklch-aware contrast census found every text node ≥4.5:1 in all three themes; the one unadapted control was the theme switcher itself (raw ids, English tips — now translated), and terminal, the theme no baseline had ever looked at, joins the visual specs | **shipped** — `160b0ef5`, `c471f700`, `3f0a044d` |

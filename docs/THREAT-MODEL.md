...
| T12 | Flying agent reacts to a (possibly stale) red-main signal by running its own `git revert` cascade on the primary checkout, hunting backward for a green baseline | Any Bash `git revert` targeting a specific SHA, `HEAD~N`/`HEAD^`, a branch/tag, or a range — observed 2026-09-06: a live flight's final firing saw `converged branch 'main' e2e is red` and ran a nine-deep revert cascade that destroyed already-landed fixes (a sibling's own feature, a doc slice already on `origin/main`) because every red verdict it walked back over was stale (`docs/debriefs/2026-09-06-red-main-revert-cascade.md`) | CLI-level guard (`guard.ts` `checkDestructiveGit`'s `isSelfRevertOfHead`) allows only a bare `git revert HEAD` (the flight undoing the commit it just made) and denies any other revert target, same `PreToolUse` layer as T1/T2 | Textual guard only — cannot distinguish a legitimate `HEAD` self-correction from one dressed up to look like it; the real backstop is that any broader revert must now be proposed as a board task instead of executed, per the debrief's "revert is an operator verb" law | **Mitigated** for the observed shape |
| T13 | Two+ flights running directly against the same primary, non-worktree checkout write into one shared `.git/index` with no locking or partitioning; a concurrent process's broader `git add` silently sweeps a scoped, unrelated `git add <path>`'s staged content into its own commit | Overlapping `git add`/`git commit` from independent flight processes against one shared checkout — reconfirmed at least six times 2026-09-06→2026-09-07 in distinct shapes: disjoint staged content swept into an unrelated commit (`docs/debriefs/2026-09-06-disjoint-staged-content-swept-into-unrelated-commit.md`), the same file swept a second time (`2026-09-06-mirror-pass-swept-a-second-time-same-file.md`), a commit race preempting an attempted landing, twice (`2026-09-06-commit-race-preempts-attempted-landing.md`, `...-recurs-a-second-time-same-session.md`), and live dirty-file churn caught mid-orientation with no destructive command run (`2026-09-07-fifth-reconfirmation-verdict-ap-mtq46c9a-2.md`) | Detection/avoidance rule shipped in `packages/engine/src/prompt.ts` (`3f4bd6c6`): an uncommitted diff present at firing start with no `wip(autopilot): checkpoint` commit may be a live sibling's, not solely a turn-capped prior firing's leftover — re-check `git status`/`git log` immediately before the final commit and treat any already-dirty file as claimed rather than a retry cue. Per-flight worktrees (the `fly-autopilot--fleet-N` convention this repo's own FLEET data already uses) sidestep the shared index entirely by giving each flight its own `.git/index` | Detection rule is agent-side discipline, not a lock — it reduces damage but does not prevent the race; no content has been lost across any reconfirmation (scoped-add's target file always lands correctly, just bundled with extra unrelated content), but the underlying architecture gap is real: nothing stops two processes from writing the same shared, non-worktree `.git/index` concurrently. Board `ap-mtm4qzty-1` (the architecture-level fix — most directly, requiring every flight to run in its own worktree rather than ever flying the primary checkout directly) remains an open, operator-owned decision | **Partial** — detection: mitigated; prevention: open, pending an operator policy decision |
| T14 | The pre-land e2e/red guard's REMEDY ESCAPE (ADR 0008, amendment 2026-09-17) admits a landing into a red converged branch when the landing touches a file the red run's own failure names — a branch could touch such a file only to pass the guard | A landing branch that edits a file named in the latest red run's log (`gh run view --log-failed`) without fixing anything | The escape is narrow and evidenced: the file must be one the failure itself names (`implicatedFilesFromFailedLog`, `remedyFilesOf` — `apps/dashboard/src/landing/execute.ts`), the local gate still runs in full (typecheck/lint/format/test/build plus the detected `ci:*` extras) before any git command, and every admission persists an `e2e-land-remedy` events row naming the files (mirror of `e2e-land-block`), so a landing that went into a red branch is never invisible | A gate-green landing that names the right file but does not actually clear the red leaves `main` red for one more cycle — the same state as before the escape existed, now with an audit row; the guard cannot prove a fix, only that the fix's file moved | **Mitigated by design** — accepted trade-off against the deadlock it closes (a guard whose refusal cannot be cleared by the fix); re-review if landings ever come from an untrusted branch author |
...
| This document last reviewed against the above | 2026-09-04 (T9 re-reviewed 2026-08-24; T11 incident 2026-08-27; T13 added 2026-09-07, catalogued from six prior debrief-level reconfirmations of `ap-mtq46c9a-2`) |

...
| `pnpm dashboard:fleet-report [folder] [days]` | One repeatable evaluation of the folder's firings over the last `days` (default 7), across every lane: what they worked on (product work, tests, docs, verdict re-checks, convergence-red repairs), how each ended, cost per shipped commit, per lane and per model; and what the convergence gate said after each sync-back — a red on a lane's own commit, a red on a merge, or no verdict. Read-only. |
...
| No gating check reported on the head at all (an empty rollup, or only "(optional)" checks) | queue-for-human — no gate verdict exists to judge, and nothing may be running: CI triggers only for PRs into `main`, a fork's first workflow run waits on your approval, and optional checks never gate; none of that is the author's to fix, so the reasoning names those possibilities instead of claiming "still running", and the next pass re-judges once a gating check reports. When the Actions run list CONFIRMS a run stuck in `action_required` (a fork's first run awaiting your approval), the card says so instead of shrugging: its badge reads "🔒 awaiting approval to run CI" rather than the generic held-for-you badge, the reasoning names the blocked run id(s) and the `gh api .../approve` command, and the card carries a "🔓 Review & approve on GitHub" link straight to the PR's Checks tab, where GitHub's own "Approve and run" button lives — a link, never an auto-approve button: gating an untrusted fork's CI run behind your own click is the point |
...
| Deletes a test file outright (the fetched diff's `deleted file mode` headers, kept where the path is a test artifact: a `test/`, `tests/`, `__tests__/`, `__snapshots__/`, or `e2e/` directory, a `*.test.*`/`*.spec.*` module, or a Playwright `*-snapshots/` baseline) | queue-for-human — the first deterministic "does it genuinely improve" verdict: removing a test makes the gate EASIER (CI runs only the tests the PR's checkout still contains, and the coverage floor catches only a large removal), so its green result cannot vouch for the change; deleting an obsolete test is legitimate, which is why this is your call rather than a request-changes; the reasoning names every deleted test path |

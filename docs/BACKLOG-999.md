...
- [~] **C5** Commit-time independent review (pre-M8 slice): one cheap fresh-context diff-review call per firing, find-problems instruction, non-blocking, finding recorded on the firing. Slice landed: `packages/engine/src/firing.ts` (port types in `ports.ts`) reviews every gate-passed firing's diff on a tool-less cheap model (`AUTOPILOT_REVIEW_MODEL`, default `haiku`, `off` to disable; wired in `fly.ts`), the result rides `FiringRecord.review` and one flight-log line (RUNBOOK §6). Remaining: a dashboard surface for review findings on the firing detail view; move the review section out of `firing.ts` into its own module together with its `pr-review` census pin (it lives in `firing.ts` only because that census file was claimed by sibling lanes when it landed).

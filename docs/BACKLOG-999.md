...
- [~] **C5** Commit-time independent review (pre-M8 slice): one cheap fresh-context diff-review call per firing, find-problems instruction, non-blocking, finding recorded on the firing. Slices landed: `packages/engine/src/commit-review.ts` (port types in `ports.ts`; `firing.ts` decides when it runs, and `pr-review`'s engine census triages it as advisory) reviews every gate-passed firing's diff on a tool-less cheap model (`AUTOPILOT_REVIEW_MODEL`, default `haiku`, `off` to disable; wired in `fly.ts`), the result rides `FiringRecord.review` and one flight-log line (RUNBOOK §6). Remaining: a dashboard surface for review findings on the firing detail view.

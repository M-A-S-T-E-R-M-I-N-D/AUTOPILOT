...
| [~] Model catalogue — every family (Fable/Opus/Sonnet/Haiku) lists an auto-tracking alias plus a pinned id, `modelFamilyOf`/`describeModel`/`tracksLatest` (`packages/engine/src/models.ts`, deliberately never gates: an unrecognized model string still flies unchanged), operator doc `docs/MODELS.md`, informational `pnpm ci:model-freshness` diffing the catalogue against the installed CLI's own advertised aliases — makes model choices LISTABLE, records which version an alias RESOLVED to, and notices a new FAMILY, none of which routing (above) alone provides; no dashboard surface consumes it yet (§I "Still unbuilt: any dashboard model picker" already tracks that gap) | RESEARCH-LIBRARY "Model economics" | M6 |
...
live `firing-v14`. The other 8 rows (the grounded-Ask/RAG split, the injection-defense fence, retrieval-as-MCP,

...
(`750bcee8`) — scoped-add is not a safe mitigation against shared-index races."

## Verification of the claim

The claim is accurate. `git show --stat 750bcee8` ("docs: reconcile 2 stale
...
silently absorbed into `750bcee8`'s commit by a concurrent process that
...
`ap-mtm4qzty-1` hazard class that day (2026-09-06): `cfc017b6` (identical-
content commit race), `cdf648fd` (mirror-pass swept a *second* time),
`4febc1aa` (two instances redundantly re-verifying the same abandoned diff),
`b75ff525` (concurrent writes fabricating a 26-test failure batch indistin­
guishable from a real regression) all landed as further distinct failure
shapes of the same gap. The chain terminated in `6cbdacc4`, which added the
...
3. Third snapshot: two more files newly dirty
   ...
   `750bcee8` swept — and `packages/tokens/src/strings.ts`), and
...
is first-hand confirmation, one day after the `6cbdacc4` mitigation landed,
...
mitigation is real but insufficient by itself; `6cbdacc4`'s detection rule

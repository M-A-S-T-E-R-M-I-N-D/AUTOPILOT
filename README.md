...
Built milestone-by-milestone per [`docs/ACTION-PLAN.md`](docs/ACTION-PLAN.md) (M0→M9). Current version **0.21.0** — see
...
## Try it — two minutes, $0

No Claude account, no API key, no tokens spent. Two commands seed a real dashboard with a real fleet and fly a
real (scripted, zero-cost) flight — real onboarding, real git commits, real gate, real telemetry:

```bash
pnpm dashboard:demo      # seed the Fleet view with 3 sample projects (real onboarding pipeline)
pnpm dashboard:flight    # actually FLY a 4th sample project — 5 real gate-verified firings, $0
pnpm dashboard:start     # open http://127.0.0.1:4317 and watch it
```

On Windows, double-click `scripts/launchers/DEMO-DASHBOARD.cmd` and `FLY-DASHBOARD.cmd` instead of the first two
commands. Full walkthrough, what's real vs. scripted, and cleanup: [`docs/SANDBOX.md`](docs/SANDBOX.md).

The screenshots below are what you'll see:
...
1337 · REL AZEUS · MΔSTERMIND. (2026). *AUTOPILOT* (Version 0.21.0) [Computer software]. https://github.com/M-A-S-T-E-R-M-I-N-D/AUTOPILOT

**IEEE**

1337 · REL AZEUS · MΔSTERMIND, "AUTOPILOT," Version 0.21.0, 2026. [Online]. Available: https://github.com/M-A-S-T-E-R-M-I-N-D/AUTOPILOT
<!-- HOW-TO-CITE:END -->
